...
## Spec Gaps (flagged, not resolved)

1. **Backoff reset-on-success**: not stated whether the 1s→30s counter resets after a successful reconnect. Assumed yes, but unconfirmed — needs a decision before connection-manager tests lock it in.
2. **`status` enum values**: proposal requires a status exists, but doesn't enumerate states (e.g. `idle|connecting|open|reconnecting|closed`). Left to design.
3. **Close code `1000`**: handoff doc calls it a backend config issue ("not a frontend retry case"), but the approved proposal's binary 4401/4403-only terminal rule makes it retryable — faithfully reflected above, but flagged as an accepted operational risk (possible silent infinite retry loop on origin misconfiguration).
4. **Malformed pushed-event payload**: no defined behavior for an unparseable/unexpected event body arriving over an established connection (distinct from documented subscribe error frames).
